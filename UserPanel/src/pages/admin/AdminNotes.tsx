...
import { Search, Filter, Download, Eye, Star, Clock, User, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
...
const AdminNotes: React.FC = () => {
  ...
  const handleApproveNote = async (id: string) => {
    try {
      await apiService.approveNote(id);
      fetchNotes();
    } catch (error) {
  ...
  const filteredNotes = notes.filter(note => {
    ...
    const matchesStatus = !selectedStatus || 
      (selectedStatus === 'approved' && note.approvedById) ||
      (selectedStatus === 'pending' && !note.approvedById);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="mt-4 sm:mt-0 flex items-center space-x-3">
          ...
          <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-orange-100 text-orange-800">
            {filteredNotes.filter(note => !note.approvedById).length} pending
          </span>
        </div>
      </div>
          ...
          <div key={note.id} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all group">
            ...
            <div className="flex items-center justify-between">
              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                ...
                {note.approvedById ? 'Approved' : 'Pending Review'}
              </span>
